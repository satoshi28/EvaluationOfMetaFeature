...
bool ConnectingDB::loadDB(double latitude, double longitude, std::vector<Pattern>& patterns)
{
	...
	//取得用データテーブル
	System::Data::DataTable^ descTable = gcnew System::Data::DataTable("data");
	System::Data::DataTable^ keyPointTable = gcnew System::Data::DataTable("data");
	...
	try{
		...
		OleDbDataAdapter^ keypointAdapter = gcnew OleDbDataAdapter(strCommandKeypoint, strConn);
		OleDbCommandBuilder^ keypointBuilder = gcnew OleDbCommandBuilder(keypointAdapter);
		...
		SaveToCSV(descTable, filename, true,separator ,quote, replace);
		
		loadFeatureIntoPattern(descTable, patterns);

		//loadKeypointsIntoPattern(keyPointTable, patterns);
		return true;
	}
...
void ConnectingDB::loadKeypointsIntoPattern(System::Data::DataTable^ table, std::vector<Pattern>& patterns)
{
	...
	for(int i = 0; i < table->Rows->Count; i++)
	{
		...
		if(id != System::Convert::ToInt32( datarow["ID"]->ToString()) )
		{
			...
			patterns[count].keypoints = keypoints;
			count++;
			//初期化
			num = 0;
			id= System::Convert::ToInt32( datarow["ID"]->ToString() );
		...
		keypoint.pt.x =  System::Convert::ToSingle( datarow["px"]->ToString() );
		keypoint.pt.y = System::Convert::ToSingle( datarow["py"]->ToString() );
